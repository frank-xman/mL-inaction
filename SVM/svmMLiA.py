...
def testRbf(k1 = 1.3):
	...
	showDataSet(datMat,labelMat,alphas)

def showDataSet(dataMat, labelMat,alphas):
	"""
	数据可视化
	Parameters:
	    dataMat - 数据矩阵
	...
	for i,alpha in enumerate(alphas):
		if abs(alpha)>0:
			data_find.append(dataMat[i])
		else: continue
	plt.scatter(np.transpose(data_find)[0],np.transpose(data_find)[1],c='red')
	plt.show()
